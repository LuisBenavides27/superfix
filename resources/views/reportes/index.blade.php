<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <h3 class="pt-4 text-2xl text-center">GESTION DE REPORTES PARA ACTIVOS ENVIADOS A REPARACION</h3>
        <br>
        @if (session('info'))
                ...
                <a href="#"
                    ...
                    <div class="w-full">
                        ...
                        <p class="mt-4 text-gray-500 text-sm leading-relaxed justify text-justify">
                            En esta seccion podras crear un reporte pdf de los activos enviados a reparacion que ya han
                            sido reparados, selecciona la fecha de inicio y fecha final para generar el reporte segun sea requerido.
                        </p>
                        <br>
                        <form action="{{ route('reportes.create') }}" method="post" id="reporte-form">
                            ...
                            <div class="mb-4">
                                ...
                                <input required type="date" class="w-full border-gray-300 rounded-md"
                                    name="fecha_inicial" max="{{ date('Y-m-d') }}" id="fecha_inicio_1">
                            </div>
                            <div class="mb-4">
                                ...
                                <input type="date" required name="fecha_final"
                                    ...
                                    max="{{ date('Y-m-d') }}" id="fecha_fin_1">
                            </div>
                            <div class="flex justify-center">
                                <button id="btn-generar-reporte" type="sumbit"
    ...
    <div id="modal-espera" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        <div class="bg-white border border-gray-300 shadow-lg rounded-lg p-6 w-86 h-86">
            ...
            <h4 class="text-center">Generando reporte... Por Favor espere...</h4>
        </div>
    </div>

</x-app-layout>
<script>
    document.getElementById('reporte-form').addEventListener('submit', function(event) {
        // Verifica si los campos están vacíos
        if (!document.getElementsByName('fecha_inicial')[0].value || !document.getElementsByName('fecha_final')[0].value) {
